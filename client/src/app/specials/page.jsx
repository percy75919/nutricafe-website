...
import styles from '../menu/MenuPage.module.css'; // Reusing the same styles
...
const SpecialsPage = () => {
  ...
  useEffect(() => {
    const fetchSpecials = async () => {
      try {
        ...
        const res = await axios.get(`${API_URL}/api/menu/specials`);
  ...
  }, [specialItems, searchTerm]); // Recalculate when items change
  ...
  const specialsByCategory = filteredSpecials.reduce((acc, item) => {
  ...
  if (loading) return <div className={styles.loader}>Loading Specials...</div>;

  return (
    <section>
      <div className="container">
        <div className="section-title">
          <h2>NutriCafe's Weekly <span>Specials</span></h2>
        </div>
        <div className={styles.filterControls}>
          <input type="text" placeholder="Search within specials..." className={styles.searchInput} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        ...
        {filteredSpecials.length > 0 ? (
            ...
            specialsByCategory[category] && (
              <div key={category} className={styles.menuCategory}>
                ...
                <div className={styles.scrollContainer}>
                  ...
                  >
                    {specialsByCategory[category].map(item => {
                      ...
                      return (
                        <div key={item._id} className={styles.menuCard}>
                          {item.isSpecial && <div className={styles.specialBadge}>SPECIAL</div>}
          ...
          <p style={{ textAlign: 'center', marginTop: '40px' }}>No specials match your search.</p>
        )}
      </div>
    </section>
  );
